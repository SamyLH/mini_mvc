...
final class Router
{
    ...
    public function dispatch(string $method, string $uri): void
    {
        ...
        // 3. Astuce : On enlève le dossier racine de l'URL demandée
        // Si l'URL commence par le dossier du script, on le coupe.
        if (strpos($path, $scriptName) === 0) {
            $path = substr($path, strlen($scriptName));
        }
        
        // Si le chemin est vide après découpage, c'est la racine '/'
        if ($path === '' || $path === false) {
            $path = '/';
        }
